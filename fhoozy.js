...
const { channels, staffRoles, unregisterRoles, fhoozyunregisterSound, fhoozystaffSound, tokens } = require("./settings.json");
 ...
 tokens.forEach((token, i) => {
  ...
  client.on("voiceStateUpdate", async (oldState, newState) => {
    ...
    else if (hasStaff && staffSize === 1 && unregisterSize === 1) await connection.play(fhoozystaffSound);
  });

  
  client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Rhovanion ❤️ Fhoozy" }, status: "idle" });
  })
